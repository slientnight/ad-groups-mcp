...
For standalone HTML compliance reports from live AD data (with CLI flags for `--no-db`, `--policy-file`, `--output`), see the [ad-group-audit](https://github.com/example-org/ad-group-audit) repository.
...
## Relationship to ad-group-audit

This repository is the MCP server half of a two-repo split. The standalone CLI audit tool lives in the [ad-group-audit](https://github.com/example-org/ad-group-audit) repository.

Both repos are fully independent — neither declares a dependency on the other. They share a set of core modules (`models.py`, `policy_engine.py`, `ad_query.py`, `report.py`, `review_resolver.py`, `sqlite_store.py`) via a copy-based approach. The canonical (primary) versions of these modules live in `ad-group-audit`. This repo keeps its own copies under `ad_groups_mcp/`.

For standalone AD group auditing without MCP or AI tooling, use the `ad-group-audit` repo directly.

## Shared Module Sync Process

The following modules in `ad_groups_mcp/` are copies of their canonical versions in the `ad-group-audit` repo:
...
When a shared module needs to change:

1. Make the change in the `ad-group-audit` repo first (canonical source).
2. Copy the updated file into `ad_groups_mcp/`.
3. Verify import paths — the `ad-group-audit` copies use `ad_group_audit.*` imports while this repo uses `ad_groups_mcp.*`. Update the import prefix after copying.
4. Run `pytest tests/ -v` in both repos to confirm nothing broke.

Sync is manual and intentional. These core modules change infrequently, so the overhead is minimal.
